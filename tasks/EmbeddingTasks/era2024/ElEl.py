...
class CleaningTaskElEl2024(EmbeddingTask):

    RequiredTask = SelectionTask2024
    
    def output(self):
        """The path to the files the cmsdriver command is going to create"""
        return law.wlcg.WLCGFileTarget(f"2024/{self.dataset}/ElEl/cleaning/{self.branch}_cleaning_{self.output_file_suffix()}.root")
    ...
    def run(self):
        ...
        self.run_cms_driver(
            ...
            datatier="RAWRECO",
            procModifiers="tau_embedding_cleaning,tau_embedding_mu_to_e",
...
class GenSimTaskElEl2024(EmbeddingTask):
    
    RequiredTask = CleaningTaskElEl2024

    def output(self):
        """The path to the files the cmsdriver command is going to create"""
        return law.wlcg.WLCGFileTarget(f"2024/{self.dataset}/ElEl/gensim/{self.branch}_gensim_{self.output_file_suffix()}.root")
    ...
    def run(self):
        ...
        self.run_cms_driver(
            ...
            datatier="RAWSIM",
            procModifiers="tau_embedding_sim,tau_embedding_mu_to_e",
            filein=",".join(self.get_input_files()),
            number=self.emb_number_of_events,
        )
...
class HLTSimTaskElEl2024(EmbeddingTask):

    RequiredTask = GenSimTaskElEl2024

    def output(self):
        """The path to the files the cmsdriver command is going to create"""
        return law.wlcg.WLCGFileTarget(f"2024/{self.dataset}/ElEl/hltsim/{self.branch}_hltsim_{self.output_file_suffix()}.root")
...
class RecoSimTaskElEl2024(EmbeddingTask):
    
    RequiredTask = HLTSimTaskElEl2024

    def output(self):
        """The path to the files the cmsdriver command is going to create"""
        return law.wlcg.WLCGFileTarget(f"2024/{self.dataset}/ElEl/recosim/{self.branch}_recosim_{self.output_file_suffix()}.root")
...
class MergingTaskElEl2024(EmbeddingTask):
    
    RequiredTask = RecoSimTaskElEl2024

    def output(self):
        """The path to the files the cmsdriver command is going to create"""
        return law.wlcg.WLCGFileTarget(f"2024/{self.dataset}/ElEl/merging/{self.branch}_merging_{self.output_file_suffix()}.root")
...
class NanoAODTaskElEl2024(EmbeddingTask):
    
    RequiredTask = MergingTaskElEl2024

    def output(self):
        """The path to the files the cmsdriver command is going to create"""
        return law.wlcg.WLCGFileTarget(f"2024/{self.dataset}/ElEl/nanoaod/{self.branch}_nanoaod_{self.output_file_suffix()}.root")
